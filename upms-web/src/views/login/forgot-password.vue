...
async function sendVerifyCode() {
  if (recoverMethod.value === 'phone') {
    ...
    if (!formData.value.account || !/^[\w.-]+@[a-z0-9.-]+\.[a-z]{2,6}$/i.test(formData.value.account)) {
  ...
  try {
    const res = recoverMethod.value === 'phone'
    ...
    MessagePlugin.success(res.devCode ? `验证码已发送至 ${res.maskedTarget}，开发验证码：${res.devCode}` : `验证码已发送至 ${res.maskedTarget}`)
    countdown.value = 60
    timer = setInterval(() => {
      countdown.value--
...
async function handleSubmit() {
  ...
  if (!formData.value.account) {
    MessagePlugin.warning(recoverMethod.value === 'phone' ? '请输入手机号' : '请输入邮箱')
    return
  }
  ...
  <div class="min-h-screen bg-gradient-to-br from-[#0052d9] to-[#003a8c] flex items-center justify-center px-6">
    <div class="w-full max-w-md">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8">
        ...
        <!-- 找回方式选择 -->
        <div class="mb-6">
          <t-radio-group v-model="recoverMethod">
            <t-radio value="phone">
          ...
          <div>
            ...
            >
              <template #prefix-icon>
                <Phone v-if="recoverMethod === 'phone'" class="w-5 h-5 text-[#86909c] dark:text-gray-400" />
