-- 描述：profile_login_only_permission_cleanup
-- 作者：宇宙星星
-- 日期：2026-05-06
...
DELETE np
FROM `ui_navigation_permission` np
JOIN `iam_permission` p ON p.id = np.permission_id
WHERE p.permission_code LIKE 'profile:%';
...
SET `status` = 'INACTIVE',
    `deleted` = 1,
    `remark` = '个人中心为登录后公共能力，无需权限分配',
    `updater_id` = 10001,
    `updated_at` = CURRENT_TIMESTAMP(3)
WHERE `permission_code` LIKE 'profile:%';

UPDATE `iam_permission_module`
SET `status` = 'INACTIVE',
    `deleted` = 1,
    `remark` = '个人中心为登录后公共能力，无需权限分配',
    `updater_id` = 10001,
    `updated_at` = CURRENT_TIMESTAMP(3)
WHERE `module_code` = 'profile';
