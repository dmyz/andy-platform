package net.junanw.upms.core.security.audit.operation.web;
...
import net.junanw.upms.core.security.audit.operation.service.OperationAuditRecord;
...
public class OperationAuditFilter extends OncePerRequestFilter {
    ...
    private static final Pattern SENSITIVE_JSON_FIELD_PATTERN = Pattern.compile(
            "(?i)\"(oldPassword|newPassword|password|confirmPassword|captcha|token|authorization|secret|settingValue|verificationCode|mobile|email|code|accessToken|refreshToken)\"\\s*:\\s*\"[^\"]*\""
    );
    ...
    private String resolveResponseSummary(ContentCachingResponseWrapper response, Exception failure) {
        ...
        return truncate(maskSensitive(new String(response.getContentAsByteArray(), StandardCharsets.UTF_8).trim()));
    }
    ...
    private String maskSensitive(String value) {
        ...
        return SENSITIVE_JSON_FIELD_PATTERN.matcher(value).replaceAll("\"$1\":\"***\"");
    }
