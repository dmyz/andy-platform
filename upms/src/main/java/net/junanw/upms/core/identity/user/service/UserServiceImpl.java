...
public class UserServiceImpl extends ServiceSupport implements UserService {
    ...
    public PageResponse<UserPageItem> page(String username, String realName, String mobile, String orgName, Integer status, LocalDateTime startTime, LocalDateTime endTime, int pageNum, int pageSize) {
        QueryChain<UserEntity> query = QueryChain.of(userMapper)
                .eq(UserEntity::getDeleted, false);

        String normalizedRealName = normalizeKeyword(realName);
        if (!normalizedRealName.isBlank()) {
            query.like(UserEntity::getDisplayName, normalizedRealName);
        }
        if (status != null) {
            query.eq(UserEntity::getStatus, normalizeStatus(status));
        }
        if (startTime != null) {
            query.gte(UserEntity::getCreatedAt, startTime);
        }
        if (endTime != null) {
            query.lte(UserEntity::getCreatedAt, endTime);
        }
    ...
    public UserDetailView create(UserSaveRequest request) {
        ...
        boolean passwordResetRequired = request.password() == null || request.password().isBlank();
        ...
        savePasswordCredential(userId, defaultPassword(request.password()), passwordResetRequired);
        return toDetail(user, buildUserContext(List.of(userId)));
    }
    ...
    public List<UserRoleItem> roles(String id) {
        Long userId = parseId(id, "用户不存在");
        requireUser(userId);
        Map<String, String> roleCatalog = roleMapper.listAll().stream()
    ...
    public void assignRoles(String id, List<String> roleCodes) {
        ...
        Map<String, RoleEntity> roleCatalog = roleMapper.listAll().stream()
                .filter(item -> !Boolean.TRUE.equals(item.getDeleted()))
    ...
    public UserImportResult importUsers(MultipartFile file) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                AccountEntity existingAccount = QueryChain.of(accountMapper)
                        .eq(AccountEntity::getAccountType, AccountType.USERNAME.name())
                        .eq(AccountEntity::getNormalizedIdentifier, AccountNormalizer.normalizeUsername(username))
                        .get();
    ...
    @Override
    @Transactional
    public void updateProfile(Long userId, String displayName, String employeeNo, String gender, String remark) {
        UserEntity user = requireUser(userId);
        user.setDisplayName(displayName.trim());
        user.setEmployeeNo(trimToNull(employeeNo));
        user.setGender(defaultGender(gender));
    ...
    @Override
    @Transactional
    public void updateAvatar(Long userId, Long avatarFileId) {
        UserEntity user = requireUser(userId);
    ...
    @Override
    @Transactional
    public void upsertVerifiedContact(Long userId, VerificationTargetType targetType, String identifier, String normalizedIdentifier) {
        requireUser(userId);
        AccountType accountType = AccountType.fromVerificationTargetType(targetType);
        AccountEntity existing = QueryChain.of(accountMapper)
        ...
        upsertAccount(userId, accountType, identifier, normalizedIdentifier, false, true, "ACTIVE");
    }
    ...
    private void validateRequest(UserSaveRequest request, Long currentUserId, String originalUsername) {
        ...
        AccountEntity existingUsername = QueryChain.of(accountMapper)
                .eq(AccountEntity::getAccountType, AccountType.USERNAME.name())
        ...
        AccountEntity existingMobile = QueryChain.of(accountMapper)
                .eq(AccountEntity::getAccountType, AccountType.MOBILE.name())
                .eq(AccountEntity::getNormalizedIdentifier, normalizedMobile)
                .get();
        ...
        if (normalizedEmail != null) {
            AccountEntity existingEmail = QueryChain.of(accountMapper)
                    .eq(AccountEntity::getAccountType, AccountType.EMAIL.name())
                    .eq(AccountEntity::getNormalizedIdentifier, normalizedEmail)
                    .get();
        ...
        if (jobNumber != null) {
            UserEntity existingUser = QueryChain.of(userMapper)
                    .eq(UserEntity::getEmployeeNo, jobNumber)
                    .eq(UserEntity::getDeleted, false)
                    .get();
    ...
    private OrganizationUnitEntity requireOrg(Long orgId) {
        OrganizationUnitEntity org = QueryChain.of(orgUnitMapper)
                .eq(OrganizationUnitEntity::getId, orgId)
    ...
    private void saveOrUpdateAccounts(Long userId, String originalUsername, String username, String mobile, String email, String status) {
        ...
        if (trimToNull(email) == null) {
            AccountEntity emailAccount = QueryChain.of(accountMapper)
                    .eq(AccountEntity::getUserId, userId)
                    .eq(AccountEntity::getAccountType, AccountType.EMAIL.name())
                    .get();
    ...
    private void savePrimaryMembership(Long userId, Long orgId, String positionName, String status) {
        ...
        if (membership.getCreatedAt() == null) {
            orgMembershipMapper.save(membership);
        } else {
            orgMembershipMapper.update(membership);
        }
    }
    ...
    private void savePasswordCredential(Long userId, String password, boolean passwordResetRequired) {
        PasswordCredentialEntity credential = QueryChain.of(passwordCredentialMapper)
                .eq(PasswordCredentialEntity::getUserId, userId)
                .get();
        if (credential == null) {
        ...
        if (credential.getCreatedAt() == null) {
            passwordCredentialMapper.save(credential);
        } else {
    ...
    private List<String> roleCodes(Long userId) {
        Map<Long, RoleEntity> roleMap = roleMapper.listAll().stream()
                .filter(item -> !Boolean.TRUE.equals(item.getDeleted()))
                .collect(Collectors.toMap(RoleEntity::getId, Function.identity(), (left, right) -> left));
        return QueryChain.of(roleBindingMapper)
    ...
    private UserContext buildUserContext(Collection<Long> userIds) {
        ...
        if (ids.isEmpty()) {
            return new UserContext(Map.of(), Map.of(), Map.of());
        }

        List<AccountEntity> accounts = QueryChain.of(accountMapper)
                .in(AccountEntity::getUserId, ids)
                .list();
        Map<Long, List<AccountEntity>> accountMap = accounts.stream().collect(Collectors.groupingBy(AccountEntity::getUserId));

        Map<Long, OrganizationMembershipEntity> membershipMap = QueryChain.of(orgMembershipMapper)
                .in(OrganizationMembershipEntity::getUserId, ids)
                .eq(OrganizationMembershipEntity::getDeleted, false)
                .list().stream()
